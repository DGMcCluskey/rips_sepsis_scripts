...
```{r}
rf_data <- full_df[, c(3:11520, 11571)]
rf_data$mortality_28d <- rf_data$`mortality_event_28days:ch1`
rf_data <- rf_data[, -11519]
rf_data[, 1:11518] <- lapply(rf_data[, 1:11518], as.numeric)
rf_data$mortality_28d <- as.factor(rf_data$mortality_28d)
...
rf_model <- randomForest(mortality_28d ~ ., data = rf_data, importance = T, ntree = 1000, classwt = c("0" = 1, "1" = 3))
```
